...
function Icons () { 
  return ( 
  <svg class='icons' viewBox="0 0 128 128" >
...
class MenuExampleTabularOnLeft extends Component {
  ...
  render() {
    ...
    const Items = [{
      ...
      name: 'companies', svg:<Icons/>
    }, {
      name: 'links', svg: <Icons/>
    }];
      ...
      <Grid>
        <Grid.Column width={4}>
          <Menu fluid vertical tabular>
            {Items.map((item) => <Menu.Item name={item.name} onClick={this.handleItemClick} active={activeItem === item.name}> {item.svg} </Menu.Item>)}
